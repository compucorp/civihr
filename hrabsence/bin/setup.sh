...
function buildDAO() {
  ...
  [ ! -d "$EXTROOT/CRM/HRAbsence/DAO/" ] && mkdir -p "$EXTROOT/CRM/HRAbsence/DAO/"
  cp -f "$CIVIROOT/CRM/HRAbsence/DAO"/* "$EXTROOT/CRM/HRAbsence/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/HRAbsence" "$EXTROOT/CRM/HRAbsence/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
