...
function buildDAO() {
  ...
  [ ! -d "$EXTROOT/CRM/HRRecruitment/DAO/" ] && mkdir -p "$EXTROOT/CRM/HRRecruitment/DAO/"
  cp -f "$CIVIROOT/CRM/HRRecruitment/DAO"/* "$EXTROOT/CRM/HRRecruitment/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/HRRecruitment" "$EXTROOT/CRM/HRRecruitment/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
