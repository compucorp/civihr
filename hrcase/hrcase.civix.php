 ...
 */
function _hrcase_civix_civicrm_postInstall() {
  _hrcase_civix_civicrm_config();
  if ($upgrader = _hrcase_civix_upgrader()) {
 ...
 */
function _hrcase_civix_civicrm_angularModules(&$angularModules) {
  if (!is_dir(__DIR__ . '/ang')) {
    return;
  }

  $files = _hrcase_civix_glob(__DIR__ . '/ang/*.ang.php');
  foreach ($files as $file) {
    $name = preg_replace(':\.ang\.php$:', '', basename($file));
    $module = include $file;
    if (empty($module['ext'])) {
      $module['ext'] = 'org.civicrm.hrcase';
    }
 ...
 */
function _hrcase_civix_insert_navigation_menu(&$menu, $path, $item) {
 ...
 */
function _hrcase_civix_navigationMenu(&$nodes) {
  if (!is_callable(array('CRM_Core_BAO_Navigation', 'fixNavigationMenu'))) {
    _hrcase_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _hrcase_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _hrcase_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _hrcase_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _hrcase_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
 ...
 */
function _hrcase_civix_civicrm_alterSettingsFolders(&$metaDataFolders = NULL) {
  static $configured = FALSE;
  if ($configured) {
    return;
  }
