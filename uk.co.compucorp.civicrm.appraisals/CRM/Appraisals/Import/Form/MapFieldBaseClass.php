 ...
 */
class CRM_Appraisals_Import_Form_MapFieldBaseClass extends CRM_Import_Form_MapField {
  ...
  public function buildQuickForm() {
    ...
    $this->addFormRule(array('CRM_Appraisals_Import_Form_MapFieldBaseClass', 'formRule'), $this);
    //-------- end of saved mapping stuff ---------

    $this->_appraisalCycleType = CRM_Core_PseudoConstant::get('CRM_Appraisals_DAO_AppraisalCycle', 'type_id');
    ...
    foreach ($mapperKeys as $key) {
      $options = NULL;
      if ($key == 'type_id'){
        $options = $this->_appraisalCycleType;
      }
      $sel2[$key] = $options;
    }
  ...
  public function postProcess() {
    ...
    $subMapper = array();
    $appraisalCycleType = CRM_Core_PseudoConstant::get('CRM_Appraisals_DAO_AppraisalCycle', 'type_id');
    for ($i = 0; $i < $this->_columnCount; $i++) {
      $selOne             = CRM_Utils_Array::value(1, $mapperKeys[$i]);
      if ($selOne && is_numeric($selOne)) {
        $subMapper[$i] = $locationsVal = $appraisalCycleType[$selOne];
    ...
    if (CRM_Utils_Array::value('updateMapping', $params)) {
      ...
      for ($i = 0; $i < $this->_columnCount; $i++) {
        ...
        if (CRM_Utils_Array::value($i,$subMapper)) {
          $location_id = array_keys($appraisalCycleType, $subMapper[$i]);
