...
abstract class CRM_Appraisals_Import_Parser extends CRM_Import_Parser {
  ...
  function setActiveFields($fieldKeys) {
    ...
    foreach ($fieldKeys as $key) {
      if (empty($this->_fields[$key]) || $key == "do_not_import") {
        $this->_activeFields[] = new CRM_Appraisals_Import_Field('', ts('- do not import -'));
      }
      else {
  ...
  function addField($name, $title, $type = CRM_Utils_Type::T_INT, $headerPattern = '//', $dataPattern = '//') {
    if (empty($name) || $name == "do_not_import") {
      $this->_fields['doNotImport'] = new CRM_Appraisals_Import_Field($name, $title, $type, $headerPattern, $dataPattern);
    }
    else {
      foreach($this->_entity as $entity) {
        ...
        if (array_key_exists("$name", $tempField)) {
          $this->_fields[$name] = new CRM_Appraisals_Import_Field($name, $title, $type, $headerPattern, $dataPattern);
