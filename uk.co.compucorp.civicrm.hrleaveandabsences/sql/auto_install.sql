...
CREATE TABLE `civicrm_hrleaveandabsences_leave_request` (
     ...
     `from_date` datetime NOT NULL   COMMENT 'The date and time the leave request starts.',
     `from_date_type` int unsigned   COMMENT 'One of the values of the Leave Request Day Type option group',
     `to_date` datetime NOT NULL   COMMENT 'The date and time the leave request ends',
     ...
     `from_date_amount` decimal(20,2)   COMMENT 'The balance change amount to be deducted for the leave request from date',
     `to_date_amount` decimal(20,2)   COMMENT 'The balance change amount to be deducted for the leave request to date',
    PRIMARY KEY ( `id` ),
...
-- /*******************************************************
-- *
-- * civicrm_hrleaveandabsences_leave_balance_change_expiry_log
-- *
-- * Leave Balance Change Expiry Log
-- *
-- *******************************************************/
CREATE TABLE `civicrm_hrleaveandabsences_leave_balance_change_expiry_log` (
...
CREATE TABLE `civicrm_hrleaveandabsences_calendar_feed_config` (
     ...
     `hash` varchar(50) NOT NULL   COMMENT 'The Calendar Feed Config Hash. Should be unique per config',
     `is_active` tinyint   DEFAULT 1 COMMENT 'Whether the feed is active or not',
     `created_date` datetime    COMMENT 'The date and time this Calendar Feed Config was created',
    PRIMARY KEY (`id`)
)  ENGINE=InnoDB DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci  ;
