 ...
 * @return array(string)
 */
function _hrleaveandabsences_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_hrleaveandabsences_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
...
function _hrleaveandabsences_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_hrleaveandabsences_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
...
function _hrleaveandabsences_civix_civicrm_angularModules(&$angularModules) {
  ...
  $files = _hrleaveandabsences_civix_glob(__DIR__ . '/ang/*.ang.php');
 ...
 */
function _hrleaveandabsences_civix_glob($pattern) {
  $result = glob($pattern);
 ...
 */
function _hrleaveandabsences_civix_navigationMenu(&$nodes) {
  if (!is_callable(array('CRM_Core_BAO_Navigation', 'fixNavigationMenu'))) {
    _hrleaveandabsences_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _hrleaveandabsences_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
...
function _hrleaveandabsences_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _hrleaveandabsences_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
