...
<div class="crm-block crm-form-block crm-activity-import-summary-form-block">
 ...
 <div class="help">
   ...
   {if $unMatchCount }
        <p class="error">
        {ts count=$unMatchCount plural='CiviCRM has detected mismatched leave IDs. These records have not been Updated.'}CiviCRM has detected mismatched leave ID. This record have not been updated.{/ts}
        </p>
        <p class="error">
        {ts 1=$downloadMismatchRecordsUrl}You can <a href='%1'>Download Mismatched Leave records</a>. You may then correct them, and import the new file with the corrected data.{/ts}
        </p>
    {/if}
    ...
    {if $duplicateRowCount}
        ...
        {ts count=$duplicateRowCount plural='CiviCRM has detected %count records which are duplicates of existing CiviCRM leave records.'}CiviCRM has detected one record which is a duplicate of existing CiviCRM leave record.{/ts} {$dupeActionString}
        </p>
        <p {if $dupeError}class="error"{/if}>
 ...
 <table id="summary-counts" class="report">
    <tr><td class="label">{ts}Total Rows{/ts}</td>
        ...
        <td class="explanation">{ts}Total rows (leave records) in uploaded file.{/ts}</td>
    </tr>
    ...
    <tr class="error"><td class="label">{ts}Mismatched Rows (skipped){/ts}</td>
        ...
        <td class="explanation">{ts}Rows with mismatched leave IDs (NOT updated).{/ts}
            {if $unMatchCount}
    ...
    <tr class="error"><td class="label">{ts}Duplicate Rows{/ts}</td>
        ...
        <td class="explanation">{ts}Rows which are duplicates of existing Leave records.{/ts} {$dupeActionString}
            {if $duplicateRowCount}
