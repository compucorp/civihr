...
/*                    'street_number' => array(
      ...
      if (array_key_exists('fields', $table)) {
        foreach ($table['fields'] as $fieldName => $field) {
          ...
          if (!empty($field['required']) || !empty($this->_params['fields'][$fieldName])) {
            switch (true) {
              case $tableName == 'civicrm_hrjobcontract_leave' && $fieldName == 'leave_leave_type':
                  $select[] = "GROUP_CONCAT(DISTINCT hrjobcontract_leave_civireport.leave_type SEPARATOR ',') AS {$tableName}_{$fieldName}";
                break;

              case $tableName == 'civicrm_hrjobcontract_leave' && $fieldName == 'leave_leave_amount':
              ...
              case $tableName == 'civicrm_hrjobcontract_health' && $fieldName == 'health_provider':
              ...
              case $tableName == 'civicrm_hrjobcontract_health' && $fieldName == 'health_provider_life_insurance':
              ...
              case $tableName == 'civicrm_hrjobcontract_pension' && $fieldName == 'pension_type':
                $select[] = "CONCAT({$this->_aliases['civicrm_contact']}_pension.sort_name, ' (InternalID: ', hrjobcontract_pension_civireport.pension_type , ', Email: ', COALESCE({$this->_aliases['civicrm_email']}_pension.email, '-'), ', ExternalID: ', COALESCE({$this->_aliases['civicrm_contact']}_pension.external_identifier, '-'), ')') AS {$tableName}_{$fieldName}";
                break;
  ...
  function from() {
    $this->_from = "
      ...
      LEFT JOIN civicrm_contact AS {$this->_aliases['civicrm_contact']}_pension ON {$this->_aliases['civicrm_hrjobcontract_pension']}.pension_type = {$this->_aliases['civicrm_contact']}_pension.id
      LEFT JOIN civicrm_email AS {$this->_aliases['civicrm_email']}_pension ON {$this->_aliases['civicrm_contact']}_pension.id = {$this->_aliases['civicrm_email']}_pension.contact_id AND {$this->_aliases['civicrm_email']}_pension.is_primary = 1
