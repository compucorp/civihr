...
INNER JOIN  civicrm_membership membership2 ON membership1.membership_type_id = membership2.membership_type_id
  ...
  public static function moveContactBelongings($mainId, $otherId, $tables = FALSE, $tableOperations = [], $customTableToCopyFrom = NULL) {
    ...
    $mainId = (int) $mainId;
    $otherId = (int) $otherId;
    $multi_value_tables = array_keys(CRM_Hrjobcontract_Dedupe_Merger::getMultiValueCustomSets('cidRefs'));
  ...
  public static function batchMerge($rgid, $gid = NULL, $mode = 'safe', $batchLimit = 1, $isSelected = 2, $criteria = [], $checkPermissions = TRUE, $reloadCacheIfEmpty = NULL) {
    ...
    return CRM_HRJobcontract_Dedupe_Merger::merge($dupePairs, $cacheParams, $mode, $redirectForPerformance, $checkPermissions);
  }
  ...
  public static function updateMergeStats($cacheKeyString, $result = []) {
    ...
    // get previous stats
    $previousStats = CRM_Hrjobcontract_Dedupe_Merger::getMergeStats($cacheKeyString);
    ...
    // delete old stats
    CRM_Hrjobcontract_Dedupe_Merger::resetMergeStats($cacheKeyString);
  ...
  public static function merge($dupePairs = [], $cacheParams = [], $mode = 'safe',
    ...
    CRM_Hrjobcontract_Dedupe_Merger::updateMergeStats($cacheKeyString, $resultStats);
    return $resultStats;
  }
  ...
  public static function moveAllBelongings($mainId, $otherId, $migrationInfo, $checkPermissions = TRUE) {
    ...
    $relTables = CRM_Hrjobcontract_Dedupe_Merger::relTables();
    $submittedCustomFields = $moveTables = $contractBlocks = $roleBlocks = $tableOperations = $removeTables = [];
    ...
    foreach ($migrationInfo as $key => $value) {
      ...
      elseif (in_array(substr($key, 5), CRM_Hrjobcontract_Dedupe_Merger::getContactFields()) && $value != NULL) {
        $submitted[substr($key, 5)] = $value;
      }
      ...
      if (!empty($contactRoles)) {
        foreach($contactRoles as $role)  {
          ...
          if (!in_array($roleID, $roleBlocks) && in_array($contractID, $contractBlocks))  {
            civicrm_api3('HrJobRoles', 'delete', [
              'id' => $roleID,
            ]);
          }
        }
    ...
    CRM_Hrjobcontract_Dedupe_Merger::moveContactBelongings($mainId, $otherId, $moveTables, $tableOperations, $customTablesToCopyValues);
    unset($moveTables, $tableOperations);

    // **** Do table related removals
    if (!empty($removeTables)) {
      // **** CRM-20421
      CRM_Hrjobcontract_Dedupe_Merger::removeContactBelongings($otherId, $removeTables);
      $removeTables = [];
    }
  ...
  protected static function dedupePair(&$resultStats, &$deletedContacts, $mode, $checkPermissions, $mainId, $otherId, $cacheKeyString) {
    ...
    $migrationInfo = [];
    $conflicts = [];
    if (!CRM_Hrjobcontract_Dedupe_Merger::skipMerge($mainId, $otherId, $migrationInfo, $mode, $conflicts)) {
      CRM_Hrjobcontract_Dedupe_Merger::moveAllBelongings($mainId, $otherId, $migrationInfo, $checkPermissions);
      $resultStats['merged'][] = [
        'main_id' => $mainId,
        'other_id' => $otherId,
      ];
  ...
  public static function getConflicts(&$migrationInfo, $mainId, $otherId, $mode) {
    ...
    $rowsElementsAndInfo = CRM_Hrjobcontract_Dedupe_Merger::getRowsElementsAndInfo($mainId, $otherId, FALSE);
