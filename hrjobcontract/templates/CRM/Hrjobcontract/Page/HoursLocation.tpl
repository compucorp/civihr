...
{if $action eq 1 or $action eq 2 or $action eq 8}
   {include file="CRM/Hrjobcontract/Form/HoursLocation.tpl"}
{else}
{if $rows}
  <div id="ltype">
     {*<div id="help">
  ...
  {* handle enable/disable actions*}
   {include file="CRM/common/enableDisableApi.tpl"}
      ...
      <a href="{crmURL q="action=add&reset=1"}" class="button"><span><div class="icon add-icon"></div>{ts}Add Standard Full Time Hours{/ts}</span></a>
        </div>
        {/if}
    </div>
  </div>
{else}
    <div class="messages status no-popup">
        ...
        {ts 1=$crmURL}There are no Hours Location entered. You can <a href='%1'>add one</a>.{/ts}
    </div>
{/if}
{/if}
