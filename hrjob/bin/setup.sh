...
function buildDAO() {
  ...
  [ ! -d "$EXTROOT/CRM/HRJob/DAO/" ] && mkdir -p "$EXTROOT/CRM/HRJob/DAO/"
  cp -f "$CIVIROOT/CRM/HRJob/DAO"/* "$EXTROOT/CRM/HRJob/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/HRJob" "$EXTROOT/CRM/HRJob/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
