  ...
  .factory('taBrowserTag', [function () {
    ...
    return function (val) {
      ...
      angular.forEach(taCustomRenderers, function (renderer) {
        ...
        if (renderer.selector && renderer.selector !== '') { elements = element.find(renderer.selector); }
        /* istanbul ignore else: shouldn't fire, if it does we're ignoring everything */
        else if (renderer.customAttribute && renderer.customAttribute !== '') { elements = taDOM.getByAttribute(element, renderer.customAttribute); }
              ...
              if (!_foundBlockElement) {
                ...
                for (i = 0; i < _children.length; i++) {
                  ...
                  if (nodeName === '#comment') {
                    ...
                    if (_subVal.trim() !== '') { value += '<' + attrs.taDefaultWrap + '>' + _subVal + '</' + attrs.taDefaultWrap + '>'; } else value += _subVal;
                  } else {
      ...
      link: function (scope, element, attrs) {
        ...
        scope.updateToolDisplay = function (key, _newTool, forceNew) {
          ...
          if (toolInstance) {
            ...
            if (_newTool.buttontext === null && _newTool.iconclass === null && _newTool.display === null) { throw ('textAngular Error: Tool Definition for updating "' + key + '" does not have a valid display/iconclass/buttontext value'); }
