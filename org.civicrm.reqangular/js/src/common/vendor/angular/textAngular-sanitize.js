 ...
 */
  function htmlParser (html, handler) {
    ...
    function parseStartTag (tag, tagName, rest, unary) {
      tagName = angular.isString(tagName) ? tagName.toLowerCase() : tagName;
 ...
 */
  ...
  function validStyles (styleAttr) {
    ...
    angular.forEach(styleArray, function (value) {
      var v = value.split(':');
      if (v.length == 2) {
        var key = trim(angular.isString(v[0]) ? v[0].toLowerCase() : v[0]);
        var value = trim(angular.isString(v[1]) ? v[1].toLowerCase() : v[1]);
        if (
 ...
 */
  function htmlSanitizeWriter (buf, uriValidator) {
    ...
    return {
      start: function (tag, attrs, unary) {
        tag = angular.isString(tag) ? tag.toLowerCase() : tag;
        ...
        if (!ignore && validElements[tag] === true) {
          ...
          angular.forEach(attrs, function (value, key) {
            var lkey = angular.isString(key) ? key.toLowerCase() : key;
