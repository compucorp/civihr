...
function buildDAO() {
  ...
  [ ! -d "$EXTROOT/CRM/Contactaccessrights/DAO/" ] && mkdir -p "$EXTROOT/CRM/Contactaccessrights/DAO/"
  cp -f "$CIVIROOT/CRM/Contactaccessrights/DAO/"/* "$EXTROOT/CRM/Contactaccessrights/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/Contactaccessrights" "$EXTROOT/CRM/Contactaccessrights/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
