  ...
  function ContractService(Api, Model, ContactDetails, $q, $log) {
    ...
    factory.getPrimary = function () {
      return this.get().then(function (response) {
        ...
        angular.forEach(response, function (contract) {
          if (contract.is_primary === '1' && contract.is_current === '1') primary = contract;
        });
    ...
    factory.getContractDetails = function (id) {
      ...
      return Api.get('HRJobDetails', data)
        .then(function (response) {
          if (response.values.length === 0) return $q.reject('No details found for contract revision with ID ' + id);
